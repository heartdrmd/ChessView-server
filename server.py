...
Endpoints:
  ...
  POST /analyze         - Full analysis (multipart image upload)
                          Optional: force_pipeline=book_bw|color to override Z0
...
Pipeline:
  ...
  6. Z0.pt → Decide pipeline (book_bw or color) OR use force_pipeline param
...
def detect_board_type(warped_img):
    ...
    """
    if Z0_MODEL is None:
        return 'color', 0.0  # Default to color pipeline if Z0 not available
...
def create_hybrid_grid(cv_corners, pattern, full_size, margin):
    ...
    anchor_point = cv_grid[anchor_cv_i, anchor_cv_j]
    
    for i in range(9):
        for j in range(9):
            if (i, j) not in cv_indices:
                delta_i = i - anchor_grid_i
                delta_j = j - anchor_grid_j
                grid[i, j] = anchor_point + delta_j * avg_h + delta_i * avg_v
...
def extract_squares(warped, grid):
    """Step 5: Extract 64 square images from grid points"""
    squares = {}
...
def classify_squares_color(squares):
    """
    COLOR PIPELINE: Classify squares using 3.pt (piece type) + P/N/B/R/Q/K.pt (color)
    ...
    for label, sq_img in squares.items():
        ...
        ml_piece = CLASS_NAMES[idx] if idx < len(CLASS_NAMES) else 'empty'
        ...
        if ml_piece != 'empty':
            # Determine piece type (uppercase)
            piece_type = ml_piece.upper()
            
            # Step 2: Use color model (P/N/B/R/Q/K.pt)
            if piece_type in COLOR_MODELS:
                ...
                # Check for 'white' or 'W' in class name
                is_white = 'white' in color_pred.lower() or color_pred.upper() == 'W'
                
                print(f"[COLOR] {label}: {piece_type} (conf={conf:.2f}) + {piece_type}.pt → {color_pred} (conf={color_conf:.2f}) → {'W' if is_white else 'B'}")
            else:
                # No color model - use case from 3.pt
                is_white = ml_piece.isupper()
                color_conf = conf
                print(f"[COLOR] {label}: {piece_type} (conf={conf:.2f}) - no {piece_type}.pt, using case → {'W' if is_white else 'B'}")
        ...
        predictions[label] = {
            ...
            'ml_type': piece_type.upper() if ml_piece != 'empty' else 'empty',
...
def analyze():
    """Full analysis pipeline with Z0 fork or forced pipeline"""
    try:
        ...
        # Check for force_pipeline parameter (from form data or query string)
        force_pipeline = None
        if 'force_pipeline' in request.form:
            force_pipeline = request.form.get('force_pipeline')
        elif 'force_pipeline' in request.args:
            force_pipeline = request.args.get('force_pipeline')
        elif request.is_json:
            json_data = request.get_json()
            if json_data and 'force_pipeline' in json_data:
                force_pipeline = json_data.get('force_pipeline')
        
        if force_pipeline:
            force_pipeline = force_pipeline.lower()
            if force_pipeline not in ['book_bw', 'color']:
                force_pipeline = None
            else:
                print(f"[REQUEST] ⚡ Force pipeline: {force_pipeline.upper()}")
        ...
        # =====================================================================
        # STEP 9: DECIDE PIPELINE - Use force_pipeline OR Z0 detection
        # =====================================================================
        if force_pipeline:
            # User explicitly requested a pipeline
            board_type = force_pipeline
            z0_conf = 1.0  # Full confidence when forced
            print(f"[PIPELINE] ⚡ FORCED to {board_type.upper()} pipeline by client request")
        else:
            # Auto-detect using Z0.pt
            board_type, z0_conf = detect_board_type(warped)
        
        result['board_type'] = board_type
        result['board_type_confidence'] = z0_conf
        result['pipeline_forced'] = force_pipeline is not None
        ...
        if board is not None:
            ...
            print(f"[RESULT] Pipeline: {board_type}" + (" (FORCED)" if force_pipeline else ""))
...
if __name__ == '__main__':
    ...
    print(f"                  Optional: force_pipeline=book_bw|color")
    print(f"\nPipelines:")
    print(f"  COLOR (default): 3.pt + P/N/B/R/Q/K.pt")
    print(f"  BOOK_BW:         Z3.pt + Z3_X_color.pt")
    print(f"  Selector:        Z0.pt (auto) OR force_pipeline param")
