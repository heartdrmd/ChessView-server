...
# HEIC/HEIF support - allows iOS photos to be uploaded directly
...
def decode_image(file_or_base64):
    """Decode image from file upload or base64 (supports HEIC with proper color conversion)"""
    try:
        if hasattr(file_or_base64, 'read'):
            # File upload - use PIL for HEIC support
            pil_img = Image.open(file_or_base64)
        else:
            # Base64
            img_bytes = base64.b64decode(file_or_base64)
            pil_img = Image.open(BytesIO(img_bytes))
        
        # Convert ICC profile to sRGB if present (important for HEIC Display P3)
        ...
        if icc_profile:
            try:
                ...
                pil_img = ImageCms.profileToProfile(pil_img, input_profile, srgb_profile, outputMode='RGB')
            except Exception as e:
                print(f"[decode_image] ICC conversion failed, using direct: {e}")
